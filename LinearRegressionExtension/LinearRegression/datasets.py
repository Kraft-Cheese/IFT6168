...
class Example2:
    ...
    def __init__(self, dim_inv=1, dim_spu=1, n_envs=20, p_env=LogNormal(0, math.sqrt(0.5)), verbose=False, linear=True, nonlinear_fn=None, nonlinear_fn_kwargs=None):
        self.dim_inv = dim_inv
        self.dim_spu = dim_spu
        ...
        self.nonlinear_fn_kwargs = nonlinear_fn_kwargs
        if not self.linear and self.nonlinear_fn is None:
            raise ValueError("If linear=False, a nonlinear function must be provided.")
    ...
    def sample(self, n=1000, env="E0", split="train"):
        ...
        if self.linear:
            ...
            # y = x1 + e, where e is noise with variance that does not depend on the environment (homoskedastic)
            y = x1 + torch.randn(n, self.dim_inv) * self.y_sdv
            x2 = y + torch.randn(n, self.dim_spu) * sdv
        else:
            # in the nonlinear case: we need constant variance e ~ N(0, sigma^2), y = f(x, theta) + e, f(is passed as a nonlinear function of x, theta)
            x1 = torch.randn(n, self.dim_inv) * self.x1_sdv

            # y = f(x1) + e, where f is a nonlinear function of x1, and e is noise with constant variance (homoskedastic)
            y = self.nonlinear_fn(x1, **self.nonlinear_fn_kwargs) + torch.randn(n, self.dim_inv) * self.y_sdv
            x2 = y + torch.randn(n, self.dim_spu) * sdv
...
class Example2HN(Example2):
    def __init__(self, dim_inv=1, dim_spu=1, n_envs=20, p_env=HalfNormal(stddev=math.sqrt(0.5)), verbose=False, linear=True):
        super().__init__(dim_inv, dim_spu, n_envs, p_env, verbose, linear)


class Example2GB(Example2):
    def __init__(self, dim_inv=1, dim_spu=1, n_envs=20, p_env=Gumbel(mean=0.1, stddev=0.85), verbose=False, linear=True):
        super().__init__(dim_inv, dim_spu, n_envs, p_env, verbose, linear)


class Example2WB(Example2):
    def __init__(self, dim_inv=1, dim_spu=1, n_envs=20, p_env=Weibull(mean=0.4, stddev=2.5), verbose=False, linear=True):
        super().__init__(dim_inv, dim_spu, n_envs, p_env, verbose, linear)


class Example2EXP(Example2):
    def __init__(self, dim_inv=1, dim_spu=1, n_envs=20, p_env=Exponential(mean=0.5), verbose=False, linear=True):
        super().__init__(dim_inv, dim_spu, n_envs, p_env, verbose, linear)
