...
class Algorithm(torch.nn.Module):
    def __init__(self, in_features, out_features, task, hparams="default", linear=True):
        super().__init__()
        ...
        # network architecture (if linear, just a single linear layer. Otherwise, add a hidden layer and ReLU)
        if self.linear:
...
class ERM(Algorithm):
    def __init__(self, in_features, out_features, task, hparams="default", linear=True):
        ...
        super().__init__(in_features, out_features, task, hparams, linear)
...
class IRM(Algorithm):
    ...
    def __init__(
            self, in_features, out_features, task, hparams="default", version=1, linear=True):
        self.HPARAMS = {}
...
class IRMv1(IRM):
    ...
    def __init__(self, in_features, out_features, task, hparams="default", linear=True):
        super().__init__(in_features, out_features, task, hparams, version=1, linear=linear)
...
class EQRM(Algorithm):
    def __init__(self, in_features, out_features, task, hparams="default", dist=Nonparametric(), linear=True):
        self.dist = dist
        self.linear = linear
        self.HPARAMS = {}
        self.HPARAMS["lr"] = (1e-2, 10**r.uniform(-4, -2))
