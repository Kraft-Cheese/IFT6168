...
def extension():
    ...
    parser.add_argument('--hparams', type=str, default="default")
    parser.add_argument('--alphas', nargs='+', default=[str(a) for a in [0.25, 0.5, 0.75, 0.9, 0.99, int(-10**30)]])
    parser.add_argument('--vrex_lambdas', nargs='+', default=[str(l) for l in [0.25, 0.5, 0.75, 0.9, 0.99, int(-10**30)]])
    parser.add_argument('--datasets', nargs='+', default=["Example2"])
    parser.add_argument('--dim_inv', type=int, default=1)
    parser.add_argument('--dim_spu', type=int, default=1)
    parser.add_argument('--n_envs', type=int, default=1000)
    ...
    parser.add_argument('--nonlinear_fn', type=str, default="relu")
    parser.add_argument('--nonlinear_fn_kwargs', type=str, default="{}")
    args = parser.parse_args()

    # Gather experiment settings
    eqrm_alphas = [ast.literal_eval(a) for a in args.alphas]
    vrex_lambdas = [ast.literal_eval(l) for l in args.vrex_lambdas] if "VREX" in args.algorithms else []
            ...
            for data_seed in range(args.n_data_seeds):
                for alg_seed in range(args.n_alg_seeds):
                    ...
                    alg_base_call = (
                        ...
                        f"--alg_seed {alg_seed} "
                        f"--hparams {hparams}"
                        f"--linear {args.linear}"
                        f" --nonlinear_fn {args.nonlinear_fn} --nonlinear_fn_kwargs {args.nonlinear_fn_kwargs}"
                    )
                    if "QRM" in alg:
                        alg_settings = [f"--hparams_fixed alpha={a}" for a in eqrm_alphas]
                    elif "VREX" in alg:
                        alg_settings = [f"--hparams_fixed vrex_lambda={l}" for l in args.vrex_lambdas]
                    else:
                        alg_settings = [""]         # add any algorithm-specific settings/hparams here
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate experiment commands for Linear Regression experiments: reproduce or extension')
    parser.add_argument('--exp_name', type=str, default="reproduce")
    args = parser.parse_args()
    if args.exp_name == "reproduce":
        print("Generating commands to reproduce results from paper...")
        reproduce()
    else:
        print("Generating commands for extension experiments...")
        extension()