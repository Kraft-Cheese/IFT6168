...
def fit_and_save_results(alg, envs, args, results_dirname, dataset):
    ...
    try:
        # if linear, save weights and biases
        if args["linear"]:
            ...
            # save nonlinear function name and kwargs
            args["nonlinear_fn"] = args["nonlinear_fn"]
            args["nonlinear_fn_kwargs"] = json.loads(args["nonlinear_fn_kwargs"])
    except Exception as e:
        print(f"Error saving weights/biases or nonlinear function info: {args["alg"]}, {args["linear"]}, {args["nonlinear_fn"]}, {e}")
    # write results
...
def run_experiment(args):
    ...
    dataset = datasets.DATASETS[args["dataset"]](
        ...
        linear=args["linear"],
        nonlinear_fn=nonlinear_algos[args["nonlinear_fn"]] if not args["linear"] else None,
        nonlinear_fn_kwargs=json.loads(args["nonlinear_fn_kwargs"]) if not args["linear"] else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--linear', type=lambda x: x.lower() == 'true', default=False)
