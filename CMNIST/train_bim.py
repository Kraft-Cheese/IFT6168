"""
train_bim.py — Training script for the BIM (Binary Interleaved M2M) CMNIST variant.

What this experiment tests
--------------------------
Does ERM choose a complex, non-linear spurious shortcut (interleaved color
pools) over a stable but lower-accuracy invariant feature (digit shape)?

Differences from train.py
--------------------------
- Dataset     : ``get_bim_datasets`` from datasets_bim.py
- Target      : binary (2-class cross-entropy), same digit binarization as train.py
- Colors      : 10-color interleaved pool scheme (non-linear RGB decision boundary)
- Label noise : 0.25 on training envs; test envs always evaluated on clean labels
- oracle      : train_envs = (0.5, 0.5) — color is uninformative at env_p=0.5
- test_env_ms : default '0.9' (hardest OOD, anti-correlated color)

Expected results
----------------
  ERM train (env_p=0.1,0.2)  : exploits color shortcut  -> high train acc, ~50% OOD
  ERM oracle (env_p=0.5,0.5) : color uninformative       -> forced onto shape (~75%)
  EQRM                        : learns invariant feature  -> OOD acc near oracle

Everything else — architecture, optimizer, full-batch InfiniteDataLoader, ERM
pretraining, cosine LR, eval loop, checkpoint saving, result logging — is
identical to train_mc.py.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='BIM ColoredMNIST (Binary Interleaved M2M, 10-color non-linear shortcut)'
    )

    # Datasets
    parser.add_argument('--train_envs',       type=str,   default='default')
    parser.add_argument('--test_envs',        type=str,   default='0.1,0.2,0.5,0.9')
    parser.add_argument('--test_env_ms',      type=str,   default='0.9')
    ...
    # General hparams  (identical defaults to train.py / train_mc.py)
    ...
    parser.add_argument('--exp_name',         type=str,   default='bim_reproduce')
    ...
    # BIM: binary target, 2-class cross-entropy, int (long) labels
    n_targets  = 2
    loss_fn    = F.cross_entropy
    int_target = True

    # Parse train / test environment flip probabilities
    test_env_ps = tuple(float(e) for e in args.test_envs.split(","))
    if args.train_envs == 'default':
        train_env_ps = (0.1, 0.2)       # same as binary CMNIST
    elif args.train_envs == 'oracle':
        train_env_ps = (0.5, 0.5)       # env_p=0.5 -> color is uninformative (50/50 pools)
    else:
    ...
    # Probe the full [0, 1] range of flip probabilities, clean labels only.
    all_ps        = [i / 10. for i in range(11)]
    all_env_names = [str(p) for p in all_ps]
    all_envs = get_bim_datasets(
