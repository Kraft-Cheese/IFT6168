"""
train_cfmnist.py — Training script for Colored Fashion-MNIST (CFashionMNIST).

What this experiment tests
--------------------------
Does ERM exploit a simple linear color shortcut (Red vs Green) even when the
invariant feature (clothing shape) is harder than MNIST digits?  Fashion-MNIST
is a strictly harder visual base dataset: within-group variation is higher and
the shape cue alone achieves only ~75% after label noise (same ceiling as
binary CMNIST, by design).

Key differences from train.py (binary CMNIST)
---------------------------------------------
- Dataset     : ``get_cfmnist_datasets`` from datasets_cfmnist.py
- Base data   : Fashion-MNIST (60k train / 10k test, 28x28, 10 clothing classes)
- Target      : binary super-class (Apparel=0 vs Accessories/Footwear=1)
- Colors      : explicit Red / Green RGB (3-channel output)
- oracle      : train_envs = (0.5, 0.5) — color uninformative at p_e=0.5
- Loss        : F.cross_entropy with n_targets=2  (int long labels)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='CFashionMNIST — Binary colored spurious-correlation benchmark'
    )
    ...
    parser.add_argument('--exp_name',         type=str,   default='cfmnist_reproduce')
    ...
    if args.train_envs == 'default':
        ...
        train_env_ps = (0.5, 0.5)       # p_e=0.5 -> Red/Green equally likely -> uninformative
    else:
    ...
    # Probe the full [0, 1] range of flip probabilities using Fashion-MNIST test set.
    all_ps        = [i / 10. for i in range(11)]
    all_env_names = [str(p) for p in all_ps]
    all_envs = get_cfmnist_datasets(
